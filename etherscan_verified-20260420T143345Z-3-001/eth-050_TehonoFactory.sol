...
// File: contracts/TehonoFactory.sol
...
contract TehonoArtistCollection is ERC1155, Ownable {
    string public name;
    string public symbol;

    constructor(string memory _name, string memory _symbol, address _artist) 
        ERC1155("") 
        Ownable(_artist) 
    {
        name = _name;
...
contract TehonoFactory {
    event CollectionCreated(address indexed creator, address collectionAddress, string name);

    mapping(address => address[]) public userCollections;

    function createCollection(string memory _name, string memory _symbol) external {
        TehonoArtistCollection newCol = new TehonoArtistCollection(_name, _symbol, msg.sender);
        address colAddr = address(newCol);
        
        userCollections[msg.sender].push(colAddr);
        emit CollectionCreated(msg.sender, colAddr, _name);
    }

    function getCollections(address _creator) external view returns (address[] memory) {
        return userCollections[_creator];
    }
}